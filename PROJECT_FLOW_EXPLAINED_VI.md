...
Nếu người dùng chọn file, hàm `handleFileUpload()` đọc nội dung bằng API `file.text()`.

Điểm cần lưu ý: frontend hiện coi đầu vào là văn bản UTF-8. Đây chưa phải pipeline trích xuất nội dung chuyên dụng từ PDF, Word hoặc Excel. Nếu đưa trực tiếp file nhị phân vào, `file.text()` có thể không đọc được nội dung đúng cách.

Khi người dùng nhấn **Generate**, hàm `handleGenerate()` thực hiện:

1. Chuyển nội dung BD thành một file text tạm.
2. Gọi API tạo project.
3. Upload Basic Design.
4. Upload UI Design nếu người dùng cung cấp.
5. Gọi API Generate.
6. Cứ ba giây hỏi backend xem job đang ở trạng thái nào.

Các API được gọi theo thứ tự:

```text
POST /api/v1/projects
POST /api/v1/projects/{projectId}/documents/basic-design
POST /api/v1/projects/{projectId}/documents/ui-design
...
Khi upload BD, luồng gọi hàm là:
...
Nội dung được lưu thành file:

```text
backend/tmp/{project_id}/basic-design.txt
backend/tmp/{project_id}/ui-design.txt
```
...
Kết quả cuối cùng được dùng làm `sampleDesigns` cho AI.
...
1. Đặt analysis thành `approved`.
2. Đặt job thành `generating_dd`.
3. Chạy `_run_detail_design_job()` trong background.
4. Gọi `run_detail_design_phase()`.
...
```text
backend/artifacts/{project_id}/{job_id}/detail-design.json
backend/artifacts/{project_id}/{job_id}/detail-design.md
```
...
- Basic Design upload;
- UI Design upload;
- Knowledge Base sample và các ảnh trong `INPUT/input/01_UI層/**`, `INPUT/DD/**`;
...
Ảnh trong `INPUT/input/01_UI層/images/` được tìm đệ quy. Nếu ảnh và Markdown có cùng screen ID, hệ thống gộp cả hai nguồn thay vì bỏ qua ảnh.
